...
func (a *AuthWithRoles) filterNodes(nodes []services.Server) ([]services.Server, error) {
	...
	roleset, err := services.FetchRoles(a.context.User.GetRoles(), a.authServer, a.user.GetTraits())
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func (a *AuthWithRoles) GenerateUserCerts(ctx context.Context, req proto.UserCertsRequest) (*proto.Certs, error) {
	...
	case req.Username == a.context.User.GetName():
		...
		if a.context.Identity.Expires.IsZero() {
		...
		req.Expires = a.Identity.GetIdentity().Expires
...
func (a *AuthWithRoles) CreateResetPasswordToken(ctx context.Context, req CreateResetPasswordTokenRequest) (services.ResetPasswordToken, error) {
	...
	if err := a.emitter.EmitAuditEvent(a.closeCtx, &events.ResetPasswordTokenCreate{
		Metadata: events.Metadata{
		...
		ResourceMetadata: events.ResourceMetadata{
			Name:    name,
			TTL:     req.TTL.String(),
			Expires: a.Clock.Now().UTC().Add(req.TTL),
		},
	}); err != nil {
